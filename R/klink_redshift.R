...
#'
#' @param environment Required character string containing desired environment. 
#'  Valid arguments are "PROD", "DEV", or "QA"
#' @param region Required character string containing name of desired region. 
#'  Currently supported arguments are "KNA" (default), "KEU", or "AMEA"
#' @param database Optional character string containing exact database name in R 
#'  format.
#' @param server Optional character string containing exact server name in R 
#'  format (beware of /s). As of now, there is only one known server for each 
#'  environment. As such, this argument is optional and currently unused.
...
#'  (turning this off results in faster initial connections and reduction of 
#'  unnecessary data in automated processes).
#'
#' @usage klink_redshift(environment, region = "KNA", database = NULL, 
...
#' red_dev <- klink_redshift(environment = "DEV", region = "AMEA", 
...
klink_redshift <- function(environment, region = "KNA", database = NULL, 
  ...
  if(is.null(database)){
    if(region == "AMEA"){database <- "klg_nga_kamea" # AMEA only inconsistency
