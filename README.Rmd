...
klink currently covers commonly used SQL databases and non-Kortex S3 and provides a more succinct approach to calling these connections:
...
In addition to the brevity of the klink functions, end users also gain the added benefit of not having to maintain credentials in their own code and individual publications.

## Setup
In order to use klink there are a few setup requirements that will need to be completed:
...
Note: the klink_s3 function requires the paws, aws.s3, and aws.iam packages
...
Note: the klink_s3R function requires the aws.s3 package in order to use the resulting connection

![](vignettes/zoltar-hex.png){width=110px}
